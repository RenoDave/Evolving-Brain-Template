...
### 7. Retrieval (no action needed)

Retrieval is via the Google Drive MCP. The vault syncs to Drive through Drive for Desktop continuously, and every Claude surface (Cowork, Desktop, Code, mobile) reads markdown directly from Drive. The inbox processor's job ends at step 6 (commit + push). See MASTER PLAN Phase 6 for the rationale.
